                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-2">
                                    ...
                                    <button type="button" class="truncate max-w-xs text-left hover:text-blue-600"
                                        @click="window.dispatchEvent(new CustomEvent('open-detail', {detail: {id: {{ $file->id }}, name: '{{ addslashes($file->name) }}', size: '{{ $file->sizeFormatted() }}', mime: '{{ $file->mime_type }}', date: '{{ $file->created_at->format('Y-m-d H:i') }}', folder: '{{ addslashes($file->folder?->name ?? 'Root') }}', previewUrl: '{{ route('files.preview', $file) }}', downloadUrl: '{{ route('files.download', $file) }}'}}))">
                                        {{ $file->name }}
                                    </button>
                                </div>
                            </td>
